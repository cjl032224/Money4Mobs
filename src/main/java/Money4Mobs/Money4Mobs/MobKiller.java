...
import Money4Mobs.Configurations.*;
...
//    public static void setLanguage(Player player){
//        int counter = 1;
//        FileConfiguration membersCfg = Api.loadConfig(Constants.PLUGIN_MEMBERS_FILE_NAME);
//        membersCfg.set("users.user-" + counter + ".language");
//    }

    public static boolean displayKillMessage(CommandSender pa){
        int counter = 1;
        boolean showMessage = false;
        for(String users : MemberConfig.membersCfg.getConfigurationSection("users").getKeys(false)) {
            String userId = MemberConfig.membersCfg.getString("users.user-" + counter + ".userId");
            assert userId != null;
            if (pa instanceof Player) {
                Player player = (Player) pa;
                if (userId.equalsIgnoreCase(player.getUniqueId().toString())) {
                    showMessage = MemberConfig.membersCfg.getBoolean("users.user-" + counter + ".showMessage");
                }
            }
            counter++;
        }
    ...
    public static void sendKillMessage(CommandSender pa, Entity e){
        ...
        for(String users : MemberConfig.membersCfg.getConfigurationSection("users").getKeys(false)) {
            ...
            if (player.getUniqueId().toString().equals(userId)) {
                showMessage = MemberConfig.membersCfg.getBoolean("users.user-" + counter + ".showMessage");
                language = MemberConfig.membersCfg.getString("users.user-" + counter + ".language");
    ...
    public static void giveMoneyCheck(CommandSender pa, Entity e) throws IOException {
        ...
        if (pa.hasPermission("m4m.rewardMoney") || pa.isOp() || pa.hasPermission("m4m.rewardmoney")) {
            ...
            List<Mobs4MoneyPlayer> pl = Main.getPlayerList();
